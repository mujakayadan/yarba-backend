...
async def test_resume_variables_format(
    ...
    assert (
        llm_service.get_completion.called
    ...
    assert (
        len(job_description) > 0 and job_description.strip() in captured_prompt
    ...
    assert (
        "JSON" in captured_system_prompt
...
async def test_collect_portfolio_data(mock_repositories, portfolio_data):
    ...
    assert (
        deserialized == portfolio_data_result
...
async def test_prompt_service_format_template(portfolio_data, job_description):
    ...
    assert (
        '"category": "Programming Languages"' in formatted
    ...
    assert (
        "cannot be serialized" not in formatted
