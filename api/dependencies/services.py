...
def get_portfolio_chat_conversation_repository() -> PortfolioChatConversationRepository:
    """Get the portfolio chat conversation repository."""
    return PortfolioChatConversationRepository()
...
def get_portfolio_chat_service(
    ...
    portfolio_repo: PortfolioRepository = Depends(get_portfolio_repository),
    profile_repo: ProfileRepository = Depends(get_profile_repository),
    conversation_repo: PortfolioChatConversationRepository = Depends(
        get_portfolio_chat_conversation_repository
    ),
    llm_service: LLMService = Depends(get_llm_service),
...
def get_portfolio_chat_conversation_service(
    conversation_repo: PortfolioChatConversationRepository = Depends(
        get_portfolio_chat_conversation_repository
    ),
    website_repo: PortfolioWebsiteRepository = Depends(
    ...
    """Get the portfolio chat conversation owner service."""
    return PortfolioChatConversationService(
        conversation_repository=conversation_repo,
        website_repository=website_repo,
    )
