...
from api.dependencies.services import (
    get_portfolio_chat_conversation_service,
    get_portfolio_website_service,
)
...
@router.get(
    "/chat/conversations",
    response_model=PortfolioChatConversationListResponse,
    summary="List stored chat conversations",
    description=(
        "List visitor chat conversations when chatbot conversation storage is enabled."
    ),
)
async def list_chat_conversations(
    ...
    limit: int = 50,
    offset: int = 0,
    service: PortfolioChatConversationService = Depends(
        get_portfolio_chat_conversation_service
    ),
) -> PortfolioChatConversationListResponse:
    """List stored portfolio chat conversations for the authenticated user."""
    try:
...
@router.get(
    ...
    response_model=PortfolioChatConversationDetailResponse,
    summary="Get chat conversation transcript",
    description="Get the full transcript for a stored visitor chat conversation.",
)
async def get_chat_conversation(
    ...
    """Get a stored portfolio chat conversation."""
    try:
