...
class PortfolioWebsiteRepository:
    ...
    async def get_by_id(self, website_id: PydanticObjectId) -> PortfolioWebsite | None:
    ...
    async def get_by_subdomain(self, subdomain: str) -> PortfolioWebsite | None:
    ...
    async def list_published_websites(
        ...
        """
        websites = (
            await PortfolioWebsite.find(PortfolioWebsite.is_published)
            .skip(skip)
            .limit(limit)
            .to_list()
        )
