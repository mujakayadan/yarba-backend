...
class PortfolioWebsiteService:
    ...
    async def delete_website(self, user_id: PydanticObjectId) -> bool:
        ...
        deleted_chats = (
            await PortfolioChatConversationRepository().delete_by_website_id(website_id)
        )
        if deleted_chats:
            self.logger.info(
                f"Deleted {deleted_chats} stored chat conversations for {subdomain}"
            )

        # Delete from database FIRST - this must succeed
        # so user can create a new website even if AWS cleanup fails
        await self.website_repository.delete(website_id)
