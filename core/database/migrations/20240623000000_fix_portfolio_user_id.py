...
class FixPortfolioUserIdMigration(Migration):
    ...
    def upgrade(self) -> None:
        ...
        for portfolio in self.db.portfolios.find({}):
            if 'user_id' in portfolio and isinstance(portfolio['user_id'], str) and not portfolio['user_id'].startswith('ObjectId'):
                self.logger.warning(f"Found portfolio with string user_id: {portfolio['_id']}, user_id: {portfolio['user_id']}")
                ...
                try:
                    ...
                    if ObjectId.is_valid(portfolio['user_id']):
                        ...
                        self.logger.info(f"Will convert string ObjectId for portfolio {portfolio['_id']}")
                        continue
                    
                    # If not a valid ObjectId, try to find matching profile
                    matching_profile = self.db.profiles.find_one({"user_id": portfolio['user_id']})
                    ...
                    if matching_profile:
                        ...
                        self.logger.info(f"Found matching profile for {portfolio['user_id']}")
                    else:
                        self.logger.error(f"Could not find matching profile for portfolio {portfolio['_id']}, user_id: {portfolio['user_id']}")
                except Exception as e:
                    self.logger.error(f"Error processing portfolio {portfolio['_id']}: {str(e)}")
        ...
        for update in portfolios_to_update:
            ...
            self.logger.info(f"Updated portfolio {update['portfolio_id']} with ObjectId user_id")
        
        # Step 2: Remove professional_title field
        self.db.portfolios.update_many(
            {},
            {'$unset': {'professional_title': ""}}
        )
        ...
        self.logger.info("Updated portfolios schema to enforce ObjectId type for user_id")
    ...
    def downgrade(self) -> None:
        ...
        self.logger.warning("Downgrade not implemented for fix_portfolio_user_id migration") 