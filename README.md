...
2. Install [uv](https://docs.astral.sh/uv/getting-started/installation/) if needed, then install dependencies:
...
```bash
cp .env.example .env
# Edit .env with your values (MongoDB, JWT secret, Firebase, LLM keys, etc.)
```

`config/firebase_credentials.json` is supported for local development but is gitignored; prefer environment variables for production.
...
# Set log level
uv run python scripts/run_api.py --log-level debug
```
...
Linting and formatting use [Ruff](https://docs.astral.sh/ruff/) only:
...
```bash
# Generate a resume
uv run python scripts/examples/generate_resume.py
